...
import { getTasks, Task, createTask, updateTask, deleteTask } from "~/api/tasksApi";

export default function ToDoList() {

  const [text, setText] = createSignal("");
  const [uppgifter, setUppgifter] = createSignal<Task[]>([]);
  ...
  const AddUppgift = async () => {
    if (text().trim() === "") return;

    const created = await createTask({ text: text(), completed: false });
    setUppgifter([created, ...uppgifter()]);
    setText("");
  };

  async function toggleCompleted(index: number) {
    const list = uppgifter();
    const item = list[index];

    const updated = await updateTask(item.id, {
      text: item.text,
    ...
    const newList = [...list];
      ...
      setUppgifter(newList);
  }

  async function removeUppgift(index: number) {
    const list = uppgifter();
    const item = list[index];

    await deleteTask(item.id);

    setUppgifter(list.filter((_, i) => i !== index));
  }
